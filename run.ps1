...
# run mysql
docker run --name mysql -d -e MYSQL_DATABASE="$($settings.MYSQL_DATABASE)" -e MYSQL_USER="$($settings.MYSQL_USER)" -e MYSQL_PASSWORD="$($settings.MYSQL_PASSWORD)" mysql/mysql-server:5.7
